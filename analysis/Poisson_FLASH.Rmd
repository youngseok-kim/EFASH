...
```{r}
X = X = load.image("~/git/Peter/EFASH/data/images.png")[,,1,1]

EFMF_poisson <- function(X, k = 1, maxiter = 100){
...
```{r}
NMF_leeseung <- function(X, k = 10, maxiter = 500){
  ...
  update_F <- function(X,L,F){
    return( F * t(t(L) %*% (X / (L %*% t(F))) / colSums(L)) )
  }
